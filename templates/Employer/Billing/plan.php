...
<div class="d-flex align-items-center mb-2">
  ...
  <div class="row g-3">
    <?php foreach ($plans as $key => $p) : ?>
        <?php
          $isCurrent = ($company->plan === $key);
          $disableFreeBecauseSub = ($hasSub && $key === 'free' && $company->plan !== 'free');
        ?>
      <div class="col-md-4">
        <div class="card shadow-sm h-100">
          <div class="card-body d-flex flex-column">
            ...
            <div class="mt-auto">
              ...
              <?php elseif ($disableFreeBecauseSub) : ?>
                <div class="form-text mt-1">
                  「今期末で解約」または「今すぐ解約」でサブスク解除後に自動で Free に戻ります。
                </div>
                <button class="btn btn-outline-secondary w-100" disabled
                        title="サブスク解約後にFreeへ戻せます">
                  Free へ変更（サブスク解約が必要）
                </button>
              <?php else : ?>
                  <?= $this->Form->create(null, [
                        'url' => ['prefix' => 'Employer', 'controller' => 'Billing', 'action' => 'checkout', $key],
                        'method' => 'post',
                      ]) ?>
