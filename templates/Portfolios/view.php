  ...
  <?php if (!empty($portfolio->user_id) && !empty($portfolio->user)) : ?>
    ...
    <a href="<?= $this->Url->build(['controller' => 'Users', 'action' => 'profile', $portfolio->user->id]) ?>">
      <?php if (!empty($portfolio->user->icon_path)) : ?>
        <img
          src="/img/<?= h($portfolio->user->icon_path) ?>"
        ...
        <div>
          ...
          /
            ...
            <?php
                ...
                $isOtherUser = (
                  $identity &&
                  $identity->get('id') !== $portfolio->user->id
                );
            ?>
            <?php if ($isOtherUser) : ?>
        ...
        <?php if ($portfolio->purpose || $portfolio->basic_spec) : ?>
          <h5>[1] 設計構想・目的</h5>
            <?php if ($portfolio->purpose) : ?>
              <p>
                <strong>目的:</strong>
                <?= nl2br(h($portfolio->purpose)) ?>
              </p>
            <?php endif; ?>
            <?php if ($portfolio->basic_spec) : ?>
              <p>
                <strong>基本仕様:</strong>
                <?= nl2br(h($portfolio->basic_spec)) ?>
              </p>
            <?php endif; ?>
        <?php endif; ?>
          ...
          <h5 class="mt-4">[2] 設計と部品情報</h5>
            <?php if ($portfolio->design_url) : ?>
              <p>
                <strong>設計書:</strong>
                <a
                  href="<?= h($portfolio->design_url) ?>"
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <?= h($portfolio->design_url) ?>
                </a>
              </p>
            <?php endif; ?>
            <?php if ($portfolio->design_description) : ?>
              <p>
                <strong>説明:</strong>
                <?= nl2br(h($portfolio->design_description)) ?>
              </p>
            <?php endif; ?>
            <?php if ($portfolio->parts_list) : ?>
              <pre><?= h($portfolio->parts_list) ?></pre>
            <?php endif; ?>
        <?php endif; ?>

        <?php
        if (
            $portfolio->processing_method
            || $portfolio->processing_notes
            || $portfolio->analysis_method
            || $portfolio->analysis_result
        ) :
            ?>
          <h5 class="mt-4">[3] 加工・解析</h5>
            <?php if ($portfolio->processing_method) : ?>
              <p>
                <strong>加工方法:</strong>
                <?= nl2br(h($portfolio->processing_method)) ?>
              </p>
            <?php endif; ?>
            ...
            <?php if ($portfolio->processing_notes) : ?>
              <p>
                <strong>ノウハウ:</strong>
                <?= nl2br(h($portfolio->processing_notes)) ?>
              </p>
            <?php endif; ?>
            ...
            <?php if ($portfolio->analysis_method) : ?>
              <p>
                <strong>解析:</strong>
                <?= nl2br(h($portfolio->analysis_method)) ?>
              </p>
            <?php endif; ?>
            ...
            <?php if ($portfolio->analysis_result) : ?>
              <p>
                <strong>考察:</strong>
                <?= nl2br(h($portfolio->analysis_result)) ?>
              </p>
            <?php endif; ?>
        <?php endif; ?>
        ...
        <?php if ($portfolio->development_period || $portfolio->mechanical_notes || $portfolio->reference_links) : ?>
          <h5 class="mt-4">[4] 補足</h5>
            <?php if ($portfolio->development_period) : ?>
              <p>
                <strong>期間:</strong>
                <?= h($portfolio->development_period) ?>
              </p>
            <?php endif; ?>
            ...
            <?php if ($portfolio->mechanical_notes) : ?>
              <p>
                <strong>反省・工夫:</strong>
                <?= nl2br(h($portfolio->mechanical_notes)) ?>
              </p>
            <?php endif; ?>

            <?php if ($portfolio->reference_links) : ?>
              <pre><?= h($portfolio->reference_links) ?></pre>
            <?php endif; ?>
        <?php endif; ?>
